...
describe('PostgresPaymentRepository', () => {
  ...
  it('registra y actualiza cuota y préstamo dentro de una sola transacción', async () => {
    ...
    const result = await new PostgresPaymentRepository(database as never).register(input);
  ...
  it('hace rollback si una cuota no existe y no inserta el pago', async () => {
    ...
    await expect(new PostgresPaymentRepository(database as never).register(input)).rejects.toMatchObject<AppError>({ code: 'INSTALLMENT_NOT_FOUND' });
    const statements = client.query.mock.calls.map(([sql]) => String(sql));
  ...
  it('anula sin borrar y devuelve el préstamo pagado a activo cuando corresponde', async () => {
    ...
    const result = await new PostgresPaymentRepository(database as never).cancel('payment-1');
